...
BASE_URL = os.environ.get('REST_API_ENDPOINT', 'https://default-url.com')  # Adicione um fallback
CERT_PATH = os.environ.get('CERT_PATH')  # Caminho do certificado do cliente
KEY_PATH = os.environ.get('KEY_PATH')  # Caminho da chave privada do cliente
...
VERIFY = CA_CERT_PATH if CA_CERT_PATH else True  # Pode ser False para ignorar SSL (não recomendado)
...
def create_database():
    ...
    try:
        response = requests.post(url, data=payload, headers=HEADERS, cert=CERT, verify=VERIFY)
